...
;(function (global, factory) {
	...
	var promiseMode = false, promiseArray = [];
	var filterArray = [];
	...
	db.filter = function(col, sign, val, fmt){
		...
		filterArray.push({column:col,sign:sign,value:(val || ""),format:(fmt || "")});
	};
			...
			if(option.folder && option.target){
				if(html5UploadPossible){
					$('#'+option.target).on('change',function(e){
						...
						$.ajax({url:uploadPath,method:'post',data:formData,dataType:'json',contentType:false,processData:false})
					...
					$('#'+option.target).on('change',function(e){
						...
						$input.wrap($('<form/>',{id:jaTarget,action:uploadPath,method:'post',enctype:'multipart/form-data',target:jaTarget}));
	...
	db.format = {
		...
		*/
		innerJoin : function(left, right, by, select, omit){
			...
			if (select){
				select.map(function(x){select[x] = 1;});
			}
			function fields(it) {
				var f = {}, k;
				for (k in it) {
	...
	function dummyInput(n, v, t){
		return $('<input/>',{type:'hidden',name:n,value:v,class:'JA_Dummy'+t});
	}
	...
	function server(method, params, url){
		...
		var _ajax = $.ajax({
				...
				data:params,
				dataType: 'json',
				headers:{'x-ja-callfrom':document.location.href, 'x-ja-calltime':(Math.round(new Date().getTime()/1000)), "cache-control": "no-cache"}
		});
